...
public class InfoStoryViewModel extends ViewModel {
    ...
    public void setInfoStory(final String urlStory) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                InfoStoryModel data = fetchInfoStory(urlStory);
                _infoStory.postValue(data);
    ...
    private void setSpinner(Boolean loading) {
    ...
    private InfoStoryModel fetchInfoStory(String urlStory) {
        ...
        return new InfoStoryModel(imageStory, nameStory, authorStory, catStory, descriptionStory, urlFirstChapter, truyenId);
    }
}
